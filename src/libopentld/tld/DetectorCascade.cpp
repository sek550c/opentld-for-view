...
void DetectorCascade::detect(const Mat &img)
{
    ...
    for(int i = 0; i < numWindows; i++)
    {
        ...
        if(!varianceFilter->filter(i)) //accept high variance, reject low variance
        {
            detectionResult->posteriors[i] = 0;
            continue;
        }

        if(!ensembleClassifier->filter(i)) // accept fern classification probability > 0.5
        {
            continue;
        }

        if(!nnClassifier->filter(img, i)) // accept patch classified to positive class (confidence > 0.65)
        {
            continue;
        }
