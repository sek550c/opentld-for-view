...
{
    Rect *trackerBB = medianFlowTracker->trackerBB; // track rectangle
    int numClusters = detectorCascade->detectionResult->numClusters; // num of detection
    Rect *detectorBB = detectorCascade->detectionResult->detectorBB; // detect rectangle
    ...
    if(trackerBB != NULL)
    {
        ...
        {
            currBB = tldCopyRect(trackerBB); // line 7
    ...
    else if(numClusters == 1) // object disappears, appears and then found by detector
    {
        ...
        currBB = tldCopyRect(detectorBB); // line 15 reinitialisation
        currConf = confDetector; 
    }
...
void TLD::initialLearning()
{
    ...
    detectorCascade->varianceFilter->minVar = initVar / 2; // init min variance of variance filter
...
void TLD::learn()
{
    ...
    //This is the positive patch
    NormalizedPatch patch;
    tldExtractNormalizedPatchRect(currImg, currBB, patch.values); //get normalized patch

    float *overlap = new float[detectorCascade->numWindows];
    tldOverlapRect(detectorCascade->windows, detectorCascade->numWindows, currBB, overlap); // return overlap (intersection/union) 
    ...
    for(size_t i = 0; i < negativeIndices.size(); i++)
    {
        int idx = negativeIndices.at(i);
        //TODO: Somewhere here image warping might be possible
        // update ensemble classifier probability negative
    ...
    for(int i = 0; i < numIterations; i++)
    {
        int idx = positiveIndices.at(i).first;
        //TODO: Somewhere here image warping might be possible
        // update ensemble classifier probability positive
        detectorCascade->ensembleClassifier->learn(&detectorCascade->windows[TLD_WINDOW_SIZE * idx], true, &detectionResult->featureVectors[detectorCascade->numTrees * idx]);
    }
    ...
    detectorCascade->nnClassifier->learn(patches); // update template classifier alg5 line 13/18
