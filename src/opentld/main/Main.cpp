...
void Main::doWork()
{
    ...
    while(imAcqHasMoreFrames(imAcq)) //main part run all time by zcy
    {
        double tic = cvGetTickCount(); // timer by zcy
        ...
        {
            tld->processImage(cvarrToMat(img));//processing by zcy
        }
        else
        {
        ...
        float fps = 1 / toc; // fps calculate by zcy
        ...
        if(showOutput || saveDir != NULL)
        {
	     ...
	     // string on screen by zcy
            ...
            if(tld->currBB != NULL)
            {
                CvScalar rectangleColor = (confident) ? blue : yellow; // confident->blue 
                cvRectangle(img, tld->currBB->tl(), tld->currBB->br(), rectangleColor, 8, 8, 0); //draw rectangle
            ...
            if(showOutput) //function key by zcy
            {
                gui->showImage(img);
                char key = gui->getKey();
